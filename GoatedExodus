...
local DefaultShootPos = UDim2.new(1, -190, 1, -180) -- Top Left of Jump
local DefaultGunPos = UDim2.new(1, -100, 1, -190)   -- Top of Jump
local DefaultThrowPos = UDim2.new(1, -195, 1, -100)  -- Left of Jump
local DefaultBombPos = UDim2.new(1, -100, 0, 100) -- Semi Top Right of Screen
...
local function getMurderer()
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= lp and v.Character and (v.Character:FindFirstChild("Knife") or v.Backpack:FindFirstChild("Knife")) then return v end
    end
    return nil
end
...
local function fireGun()
    ...
    -- 🔥 Dynamic prediction (better aim)
    local velocity = targetHRP.Velocity
    local predictedPos = targetHRP.Position + (velocity * 0.12)

    local origin = targetHRP.Position
    local cf = CFrame.new(origin, predictedPos)
...
local function toggleAntiFling(state)
    if state then
        ...
        -- Detection Loop: Neutralizes others
...
-- Helper to find players by role
local function getSheriff()
    for _, v in pairs(Players:GetPlayers()) do
        if v.Character and (v.Character:FindFirstChild("Gun") or v.Backpack:FindFirstChild("Gun")) then return v end
    end
    return nil
end
...
local function getMurderer()
    for _, v in pairs(Players:GetPlayers()) do
        -- Check if any other player has a knife in their hand or backpack
        if v ~= lp and v.Character and (v.Character:FindFirstChild("Knife") or v.Backpack:FindFirstChild("Knife")) then 
            return v 
        end
    end
    return nil
...
local function executeFling(target)
    ...
    if hrp and tRoot then
        ...
        local timeLimit = tick() + 1.5 -- Short burst for reliability
        repeat
            hrp.CFrame = tRoot.CFrame * CFrame.Angles(math.rad(math.random(1,360)), 0, 0)
            hrp.Velocity = Vector3.new(9e7, 9e7, 9e7)
            task.wait()
        until not target.Character or tick() > timeLimit
...
local function holdHostage()
    -- Check if you are the murderer using your existing helper
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= lp and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            -- Anchor and TP 5 studs in front of your face
...
local function applyPlayerESP(p)
    ...
    local function setup(char)
        if not char then return end
        local highlight = char:FindFirstChild("ExodusHighlight") or Instance.new("Highlight", char)
        highlight.Name = "ExodusHighlight"
        highlight.FillTransparency = 0.5
        local head = char:WaitForChild("Head", 10)
        local bill = head:FindFirstChild("ExodusBill") or Instance.new("BillboardGui", head)
        bill.Name = "ExodusBill"
        bill.Size = UDim2.new(0, 100, 0, 50); bill.AlwaysOnTop = true; bill.ExtentsOffset = Vector3.new(0, 3, 0)
        local label = bill:FindFirstChild("ExodusLabel") or Instance.new("TextLabel", bill)
        ...
        task.spawn(function()
            while char and char.Parent do
                if _G.ESPEnabled then
                    ...
                    highlight.Enabled = true; bill.Enabled = true
                    if isM then
                        highlight.FillColor = Color3.new(1, 0, 0); label.Text = "MURDERER\n▼"; label.TextColor3 = Color3.new(1, 0, 0)
                    elseif isS then
                        highlight.FillColor = Color3.new(0, 0, 1); label.Text = "SHERIFF\n▼"; label.TextColor3 = Color3.new(0, 0, 1)
                    else
                        highlight.FillColor = Color3.new(0, 1, 0); label.Text = "▼"; label.TextColor3 = Color3.new(0, 1, 0)
                    end
                else
                    highlight.Enabled = false; bill.Enabled = false
                end
                task.wait(0.5)
            end
        end)
...
for _, v in pairs(Players:GetPlayers()) do applyPlayerESP(v) end
...
MainTab:CreateSection("Timer/Murd Chance")
...
MainTab:CreateSection("Master Lock")
MainTab:CreateToggle({Name = "Lock/Unlock Floating UI", CurrentValue = false, Flag = "Lock_F", Callback = function(V) _G.LockAll = V end})
...
PlayerTab:CreateInput({Name = "Glitch Speed", PlaceholderText = "40", Callback = function(T) _G.SpeedGlitchValue = tonumber(T) or 50 end})
...
MiscTab:CreateToggle({
    ...
    Callback = function(Value)
        toggleAntiFling(Value) -- Calls your separated function
    end,
})
...
MiscTab:CreateButton({
    Name = "Fling Murderer",
    Callback = function()
        executeFling(getMurderer()) -- Uses existing murderer helper
    end,
})
...
MiscTab:CreateButton({
    Name = "Fling Sheriff",
    Callback = function()
        executeFling(getSheriff()) -- Uses new sheriff helper
    end,
})
